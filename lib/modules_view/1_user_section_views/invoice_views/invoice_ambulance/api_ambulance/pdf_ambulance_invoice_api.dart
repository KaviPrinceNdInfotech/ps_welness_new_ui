...
import '../../../../../controllers/invoice_controller/ambulance_controller.dart';
import '../model_ambulance/customer.dart';
import '../utils_amblnce.dart';
...
class PdfInvoiceApi {
  ...
  static Widget buildHeader(InvoiceAmblnce invoice, iconImage, ttf) => Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              buildSupplierAddress(invoice.supplieramblnce),
  ...
  static Widget buildSupplierAddress(SupplierAmb supplieramblnce) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(supplieramblnce.name,
              style: TextStyle(fontWeight: FontWeight.bold)),
          SizedBox(height: 1 * PdfPageFormat.mm),
          Text(supplieramblnce.email,
              style: TextStyle(fontWeight: FontWeight.bold)),
          SizedBox(height: 1 * PdfPageFormat.mm),
          Text(supplieramblnce.address,
              style: TextStyle(fontWeight: FontWeight.bold)),
          SizedBox(height: 1 * PdfPageFormat.mm),
          // Text(suppliernrs.mobile,
          //  style: TextStyle(fontWeight: FontWeight.bold)),
          //SizedBox(height: 1 * PdfPageFormat.mm),
          Text(supplieramblnce.pin,
              style: TextStyle(fontWeight: FontWeight.bold)),
        ],
      );
  ...
  static Widget buildInvoice(InvoiceAmblnce invoice) {
    final headers = [
      'Vehicle Name',
      // 'Date',
      'Driver Name',
  ...
  static Widget buildTotal() {
    final netTotal =
        _invoiceambController.getamblnceinvoice?.finalAmount?.toDouble();
    // invoice.items
    //     .map((item) => item.unitPrice * item.quantity)
    //     .reduce((item1, item2) => item1 + item2);
    final vatPercent = _invoiceambController.getamblnceinvoice?.gst?.toDouble();
    //invoice.items.first.vat;
    final vat = _invoiceambController.getamblnceinvoice?.gst?.toDouble();
    //double.parse(
    ...
    // 100;
    final total =
        _invoiceambController.getamblnceinvoice?.grandTotal!.toDouble();
    //netTotal + vat;
            ...
            child: Column(
              ...
              children: [
                ...
                buildText(
                  ...
                  value:
                      "${_invoiceambController.getamblnceinvoice?.gstAmount?.toDouble()}",
                  unite: true,
                ),
                // buildText(
  ...
  static Widget buildFooter(InvoiceAmblnce invoice) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: [
          ...
          SizedBox(height: 2 * PdfPageFormat.mm),
          buildSimpleText(
              title: 'Note: ', value: invoice.supplieramblnce.returnPolicy),
          SizedBox(height: 1 * PdfPageFormat.mm),
          buildSimpleText(
              title: 'Reg office: ', value: invoice.supplieramblnce.office),
        ],
      );
