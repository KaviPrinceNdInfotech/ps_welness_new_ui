...
class FrenchiesChemistDetailController extends GetxController{
  ...
  FrenchiesChemistDetailModel? getfrenchiesChemistDetailModel;
   TextEditingController?  chemistName,shopName,mobile,email,address,
      gstno,licenceno;

  void FrenchiesChemistDetailApi()async{
    isLoading(true);
    getfrenchiesChemistDetailModel = await ApiProvider.FrenchiesChemistDetailApi();
    if(getfrenchiesChemistDetailModel?.chemistRegDetail != null){
      isLoading(false);
      ///for search filter
  ...
  void FrenchiesEditChemistDetailapi(id,String chemistName,String shop,String mobile,String email,String location, String gst,String licence)async{
    isLoading(true);
    http.Response r = await ApiProvider.FrenchiesEditChemistDetail(id,
        chemistName,shop,mobile,email,location,
        gst,licence
    );
    if(r.statusCode == 200){
      isLoading(false);
