...
class UpdateBankSeperateController extends GetxController {
  final GlobalKey<FormState> updateseperatebankformkey = GlobalKey<FormState>();
  ...
  void chemistUpdateBankDetailApi() async {
    CallLoader.loader();
    http.Response r = await ApiProvider.ChemistBankSeperateDetailApi(
      AccountNo?.text,
      IFSCCode?.text,
      BranchName?.text,
    );
  ...
  void checkUpdateBankSeperateDetail() {
    if (updateseperatebankformkey.currentState!.validate()) {
      chemistUpdateBankDetailApi();
    }
    updateseperatebankformkey.currentState!.save();
  }
}
